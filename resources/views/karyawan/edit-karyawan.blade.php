...
<div class="toolbar" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
            ...
            <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Edit Data Karyawan</h1>
...
<div id="kt_content_container" class="container-fluid">
    <div class="card mb-5 mb-xl-10">
                ...
                <div class="me-7 mb-4">
                    <div class="symbol symbol-100px symbol-lg-160px symbol-fixed position-relative">
                        <div style="border-radius: 12px; overflow: hidden; max-width: 150px;">
                            <img src="{{ asset('storage/' . ($karyawan->user->img ?? 'foto_users/blank.jpg')) }}" alt="Foto Profil"
                    ...
                    <div class="d-flex justify-content-between align-items-start flex-wrap mb-2">
                        <div class="d-flex flex-column">
                            <div class="d-flex align-items-center mb-2">
                                <a href="#" class="text-gray-900 text-hover-primary fs-2 fw-bolder me-1">{{ $karyawan->user->nama }}</a>
                            </div>
                        ...
                        <!-- Tombol Hapus -->
                        <div>
                            <form action="{{ route('karyawan.destroy', $karyawan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <div id="kt_account_profile_details">
            <form method="POST" action="{{ route('karyawan.update', $karyawan->id) }}" id="edit_karyawan">
                @csrf
                @method('PUT')
                @if ($errors->any())
                ...
                <div class="card-body border-top p-6 p-lg-9">
                    <input type="hidden" name="id_users" id="id_users" value="{{ $karyawan->id_users }}">
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" id="nama" name="nama"
                                ...
                                required
                                value="{{ old('nama', $karyawan->user->nama ?? '') }}">
                        </div>
                    </div>
                    <!-- EMAIL (EDITABLE) -->
                    <div class="row mb-6">
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" id="ds_kel" name="ds_kel"
                                ...
                                value="{{ old('ds_kel', $karyawan->ds_kel) }}">
                        </div>
                    </div>
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" id="dusun" name="dusun"
                                ...
                                value="{{ old('dusun', $karyawan->dusun) }}">
                        </div>
                    </div>
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select id="marital_status" name="marital_status"
                                ...
                                <option></option>
                                <option value="Lajang" {{ old('marital_status', $karyawan->marital_status) == 'Lajang' ? 'selected' : '' }}>Lajang</option>
                                <option value="Menikah" {{ old('marital_status', $karyawan->marital_status) == 'Menikah' ? 'selected' : '' }}>Menikah</option>
                                <option value="Cerai Hidup" {{ old('marital_status', $karyawan->marital_status) == 'Cerai Hidup' ? 'selected' : '' }}>Cerai Hidup</option>
                                <option value="Cerai Mati" {{ old('marital_status', $karyawan->marital_status) == 'Cerai Mati' ? 'selected' : '' }}>Cerai Mati</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select id="status" name="status"
                                ...
                                <option value="Karyawan Tidak Tetap" {{ old('status', $karyawan->status) == 'Karyawan Tidak Tetap' ? 'selected' : '' }}>Karyawan Tidak Tetap</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select id="pend_terakhir" name="pend_terakhir"
                                ...
                                <option></option>
                                <option value="SD" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'SD' ? 'selected' : '' }}>SD</option>
                                <option value="SMP" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'SMP' ? 'selected' : '' }}>SMP</option>
                                <option value="SMA" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'SMA' ? 'selected' : '' }}>SMA</option>
                                <option value="D1" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'D1' ? 'selected' : '' }}>D1</option>
                                <option value="D2" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'D2' ? 'selected' : '' }}>D2</option>
                                <option value="D3" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'D3' ? 'selected' : '' }}>D3</option>
                                <option value="D4" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'D4' ? 'selected' : '' }}>D4</option>
                                <option value="S1" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'S1' ? 'selected' : '' }}>S1</option>
                                <option value="S2" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'S2' ? 'selected' : '' }}>S2</option>
                                <option value="S3" {{ old('pend_terakhir', $karyawan->pend_terakhir) == 'S3' ? 'selected' : '' }}>S3</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select id="gol_darah" name="gol_darah"
                                ...
                                <option></option>
                                <option value="A" {{ old('gol_darah', $karyawan->gol_darah) == 'A' ? 'selected' : '' }}>A</option>
                                <option value="B" {{ old('gol_darah', $karyawan->gol_darah) == 'B' ? 'selected' : '' }}>B</option>
                                <option value="AB" {{ old('gol_darah', $karyawan->gol_darah) == 'AB' ? 'selected' : '' }}>AB</option>
                                <option value="O" {{ old('gol_darah', $karyawan->gol_darah) == 'O' ? 'selected' : '' }}>O</option>
                            </select>
                        </div>
                    </div>
                </div>
